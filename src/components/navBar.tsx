...
export default function NavBar() {
    ...
    return (
        <header className="sticky top-0 left-0 z-50 bg-[rgba(13,13,43,0.96)] px-4 py-4 backdrop-blur md:px-8 lg:px-16">
            ...
            <button
                ...
                className="flex h-11 w-11 items-center justify-center rounded-full text-white outline-none ring-offset-2 ring-offset-[#0d0d2b] focus-visible:ring-2 focus-visible:ring-blue-300 md:hidden"
            ...
            <div
                className={`${isMenuOpen ? "flex" : "hidden"} w-full flex-col items-stretch gap-5 border-t border-white/15 pt-5 md:flex md:w-auto md:flex-row md:items-center md:gap-10 md:border-0 md:pt-0`}
                id="main-navigation-links"
            >
                <div className="flex flex-col gap-3 md:flex-row md:items-center md:gap-8">
                    {propsNavBar.map((item) => (
                        <Link
                            className="rounded py-2 text-base font-normal text-white outline-none transition hover:text-blue-300 focus-visible:ring-2 focus-visible:ring-blue-300 md:py-1"
                            href={item.href}
                            key={item.title}
                            onClick={closeMenu}
                        >
